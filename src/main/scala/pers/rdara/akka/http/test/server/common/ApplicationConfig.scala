...
class ApplicationConfig(val underlying: Config) {
  ...
  object metrics {
    ...
    val max_no_of_labels: Int = underlying.getInt(s"$metrics.max_no_of_labels")
//    val label_names: Seq[String] = underlying.getStringList(s"$metrics.label_names").asScala
    ...
    if(max_no_of_labels < 0) {
      throw new RuntimeException(s"metrics.no_of_levels (${max_no_of_labels}) can't be less than zero.)")
    }
    if(label_names.length < max_no_of_labels) {
      throw new RuntimeException(s"metrics.label_names should have at least metrics.no_of_levels (${max_no_of_labels}) entries")
    }
